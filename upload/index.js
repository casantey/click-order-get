...
var key = fs.readFileSync("/etc/letsencrypt/live/click-order-eat.bodacommunity.io/privkey.pem"); // PATH TO KEY
var cert = fs.readFileSync("/etc/letsencrypt/live/click-order-eat.bodacommunity.io/fullchain.pem"); // PATH TO CERTIFICATE
var options = {
	key: key,
	cert: cert
};
...
const storage = multer.diskStorage({
	...
	filename: function(req, file, cb) {
		...
		cb(null, datetimestamp.toString() + ".jpg");
	}
});
...
const jwtKey = "Smart2020*#1";

var serve = express();
serve.use(bodyParser.json()); //parse all requests to JSON format
serve.use(cors()); //for cross-origin access

var msg = [ {message: "Action completed successfully"} ];
var rootMsg = [ {message: "You seem lost..."} ];
var apiZMsg = [ {message: "Maybe you are lost..."} ];
...
var server = https.createServer(options, serve);

server.listen(config.port, (error) => {
	if (!error) console.log("server starting on port : " + config.port);
	else {
...
serve.use("/menu", require("./routes/menu"));
serve.use("/institution", require("./routes/inst"));
serve.use("/orders", require("./routes/orders"));
serve.use("/user", require("./routes/user"));
serve.use("/delivery", require("./routes/delivery"));
serve.use("/staff", require("./routes/staff"));

serve.get("/", verifyToken, (req, res) => {
	res.send("<h1 class='text-center' style='font-family: Montserrat;'>" + rootMsg[0]["message"] + "</h1>");
}); //END SERVE
serve.get("/api-z", verifyToken, (req, res) => {
