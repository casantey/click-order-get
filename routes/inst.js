  ...
  dbConn.query(
    ...
    (error, rows) => {
      ...
      async.map(rows, getInstitutionCategories, (err, response) => {
        async.map(
          response,
          getInstitutionDeliveryLocations,
          (err, response) => {
...
function getInstitutionDeliveryLocations(data, cb) {
  ...
  let sql = dbConn.query(`${del_loc}`, (error, rows) => {
    ...
    async.map([data], getInstitutionDeliveryRange, (err, response) => {
      ...
      return cb(null, {
        ...data,
        delivery_range: response[0].delivery_range,
        delivery_locations: rows,
      });
    });
  });
}
...
router.post("/delivery-location", xlsxTemp.single("xlsxFile"), (req, res) => {
  ...
  let inst = "PBSA002";
  let query =
    "INSERT INTO delivery_locations (location_name,delivery_price,extra_notes,branch) VALUES \n";
  data.forEach((location, index) => {
    query += `(TRIM('${location["LOCATION "]}'),TRIM(${
      location.AMOUNT
    }), TRIM('${
      location["__EMPTY_4"] ? location["__EMPTY_4"] : ""
    }'),'${inst}')`;
...
router.post("/", verifyToken, (req, res) => {
  ...
  if (!deet.branchPhoto)
    return res.status(406).send({
      code: 406,
      msg: `A photo must be provided for the institution!`,
