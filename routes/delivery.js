...
router.get("/statistics/:id", (req, res) => {
	...
	dbConn.query(`${assignedStat}${deliveringStat}${deliveredStat}`, (error, rows) => {
		if (error) {
...
router.post("/range", verifyToken, (req, res) => {
  ...
  VALUES (TRIM('${bod.rangeFrom}'),TRIM('${bod.rangeTo}'),TRIM(${bod.rangeCost}),TRIM('${req.payload
		.instCode}'),TRIM('${req.payload.username}'),NOW())`;
	dbConn.query(query, (error, rows) => {
		if (error) {
			// WHEN THERE IS AN ERROR
			saveError(error);
...
router.post("/duplicate", verifyToken, (req, res) => {
	...
	let delQuery =
		type === "location" ? `DELETE FROM delivery_locations WHERE branch=` : `DELETE FROM deliveryRange WHERE branch=`;
	let query =
		type === "location"
		...
		for (let i = 0; i < items.length; i++) {
			...
			query +=
				type === "location"
					...
					: `(TRIM('${bod.rangeFrom}'),TRIM('${bod.rangeTo}'),TRIM(${bod.rangeCost}),TRIM('${inst}'),TRIM('${req.payload
							.username}'),NOW())`;
